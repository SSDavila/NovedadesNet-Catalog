...
export default function ProductDetailModal({
  ...
  const paginate = (newIndex: number) => {
    if (newIndex < 0) setActiveIndex(displayImages.length - 1);
    else if (newIndex >= displayImages.length) setActiveIndex(0);
    else setActiveIndex(newIndex);
  };

  const getStockClasses = (stock: number) => {
    if (stock > 5) return { dot: 'bg-green-500', bg: 'bg-green-100', text: 'text-green-800' };
    if (stock >= 3) return { dot: 'bg-yellow-500', bg: 'bg-yellow-100', text: 'text-yellow-800' };
    if (stock >= 1) return { dot: 'bg-orange-500', bg: 'bg-orange-100', text: 'text-orange-800' };
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
      ...
      <AnimatePresence>
        <motion.div
          className="relative z-10 bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden"
              ...
              <>
                ...
                {displayImages.length > 1 && (
                  <>
                    <button
                      type="button"
                      onClick={() => paginate(activeIndex - 1)}
                      className="absolute left-2 z-20 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/80 rounded-full p-2 transition"
                    >
                      <FaChevronLeft />
                    </button>
                    <button
                      type="button"
                      onClick={() => paginate(activeIndex + 1)}
                      className="absolute right-2 z-20 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/80 rounded-full p-2 transition"
                    >
                      <FaChevronRight />
                    </button>
                  </>
          ...
          <div className="p-6 flex-grow overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 border-t border-gray-200">
            ...
            <div className="mt-2 flex items-center gap-3 text-sm text-gray-900">
              <span>
                ...
                <span className="font-normal">{product.prodCategory || 'N/A'}</span>
              </span>
              <span className="text-gray-300">|</span>
              <div
                className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-semibold ${stockClasses.bg} ${stockClasses.text}`}
              >
                <div className={`h-2 w-2 rounded-full ${stockClasses.dot}`} />
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 transition font-semibold"
              >
                <FaWhatsapp size={20} />
                Pedir por WhatsApp
              </button>
